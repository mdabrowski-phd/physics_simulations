...
def temperatura():
	tmp=0
	for i in xrange(particleNumber):
		tmp=tmp+dot(particles[i].v,particles[i].v)/(2*particleNumber)
	return tmp
...
while t<T:
    ...
    pre0=particleNumber*temperatura()-cisnienie()/3.
...
axis([0,650,1500,3500])
plt.plot(pre, "g-")
plt.grid(True)
plt.xlabel("czas t")
plt.ylabel("cisnienie p")
plt.savefig(folder + "parametry.pdf", format="pdf")
plt.show()
